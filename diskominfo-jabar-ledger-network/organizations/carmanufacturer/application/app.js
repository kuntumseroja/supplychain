...
const CarmanufacturerService = require('./services/carmanufacturerService.js');
...
const carmanufacturerServiceSvcInstance = new CarmanufacturerService();
...
app.post('/carmanufacturerReceived', async (req, res, next) => {
  ...
  try {
    if(!userName || userName.lenth<1) {
      return res.status(500).json("User is missing");
    } else if (!ownerName || !materialNumber) {
      return res.status(500).json("Missing requied fields: ownerName, materialNumber");
    } else {
      const result = await await carmanufacturerServiceSvcInstance.carmanufacturerReceived(userName, materialNumber, ownerName);
