...
const PartsupplierService = require('./services/partsupplierService.js');
...
const partsupplierSvcInstance = new PartsupplierService();
...
app.post('/partsupplierDistribute', async (req, res, next) => {
  ...
  try {
    if(!userName || userName.lenth<1) {
      ...
      const result = await await partsupplierSvcInstance.partsupplierDistribute(userName, materialNumber, ownerName);
