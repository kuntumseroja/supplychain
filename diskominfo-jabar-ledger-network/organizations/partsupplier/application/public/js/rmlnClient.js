...
**  Author: PJ
** JS for partsupplier web appication
**/
$(document).ready(function(){
    ...
    var urlBase = "http://54.161.38.96:30001";
     var tabs =["addToWallet", "partsupplierDistribute", "query", "queryHistory"];
     $("#queryResult").hide();
     $("#addToWalletLink").click(function(){
       showTab("addToWallet");
     });
   $("#partsupplierDistributeLink").click(function(){
     showTab("partsupplierDistribute");
   });
 ...
 $("#partsupplierDistribute").click(function(){
   var partsupplierDistributeUrl = urlBase+"/partsupplierDistribute";
   var formData = {
   ...
   $.ajax({
     type: 'POST',
     url: partsupplierDistributeUrl,
     data: formData,
     success: function(data, status, jqXHR){
       if(status==='success'){
         alert("successfully record partsupplier Distribute  in blockchain");
      }
       showTab("query");
     },
     error: function(xhr, textStatus, error){
